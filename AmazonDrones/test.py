...
def solveNQueensUtil(
    ...
    """ base case: If all queens are placed then return True - TRUE CASE"""
    if col >= N:
        if packages == maxOutput:
            packages = maxOutput
            return True

        if maxCurrentOutput < packages:
            MaxCurrentOutput = packages
    ...
    for i in range(0, N):
        if isSafe(
            ...
            coordinates = str(i) + "," + str(col)
            additional_package = 0
            if coordinates in package_map:
                additional_package = package_map[coordinates]
